...
def _print_company(c: CompanyData) -> None:
    ...
    if r["drift_regime"] == "DEFECTIVE":
        flags.append(
            "DEFECTIVE drift regime (Prop. 4.4.1 fails): "
            "mu/CCM/PIT/TTC/rating not applicable"
        )
    if r["weakly_identified"]:
        flags.append(
            f"WEAKLY_IDENTIFIED (|t| = {abs(r['drift_t_stat']):.2f} < 2): "
    ...
    t_note = (
        f"   (t = {r['drift_t_stat']:.2f})"
        if r["drift_t_stat"] is not None and r["drift_t_stat"] == r["drift_t_stat"]
        else ""
    )
    sections = [
        (
...
def _rate_impl(
    ...
    path = submission.write_submission(companies, filename=f"{c.ticker}_report_{stamp}.xlsx")
...
@app.command()
def rate(
    company: str = typer.Argument(..., help="ticker or company name"),
...
@app.command()
def batch(
    config: str = typer.Argument(..., help="path to a companies.yaml"),
