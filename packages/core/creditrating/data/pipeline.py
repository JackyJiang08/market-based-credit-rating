...
from . import alignment, cache, persistence, sectors
...
def fetch_company(ticker: str, cfg: RunConfig, rates: pd.DataFrame) -> Optional[CompanyData]:
    ...
    if data.shares_method == "shares_outstanding_single_class":
        LOG.warning(
            "  shares: market cap unavailable; fell back to "
    ...
    if not statuses:
        ...
        severity = [
            raw_errors.DataStatus.RATE_LIMITED,
    ...
    if not data.model_applicable:
        LOG.warning(
            "  %s: %s (%s) -- the structural model is not meaningful "
            "for this firm type; measures are still computed and "
            "reported, the rating is not",
            status.value,
            reason,
            firm_type.value,
        )
    ...
    # merely inapplicable.
    if (
        data.model_applicable
        and data.currency
        and data.financial_currency
        ...
        data.em_error = (
            f"statements report in {data.financial_currency}, "
            f"prices in {data.currency}; EM would mix units"
        )
        LOG.warning(
            "  MODEL_NOT_APPLICABLE: REPORTING_CURRENCY_MISMATCH "
    ...
    if (
        cfg.run_credit_model
        and not data.panel.empty
        ...
        try:
            ...
            data.drift_t_stat = m.drift / res.drift_se if res.drift_se else float("nan")
            data.weakly_identified = measures.is_weakly_identified(m.drift, res.drift_se)
            if data.weakly_identified and m.regime is measures.DriftRegime.VALID:
                LOG.warning(
                    ...
                    data.drift_t_stat,
                    sig_config.WEAK_IDENTIFICATION_T,
                )
            ...
            LOG.info(
                "  measures: RiskScore=%.2f  CCM=%.3f  mu=%.1f  DD=%.2f  "
                "EDF=%.4f  PIT_PD=%.4f",
                m.risk_score,
                m.ccm,
                m.mu,
    ...
    if data.model_applicable and data.asset_value is not None:
        total_debt_w1 = None
        if not data.panel.empty and {"ShortTermDebt", "LongTermDebt"} <= set(
            data.panel.columns
        ):
        ...
        if status is not sectors.Applicability.APPLICABLE:
            ...
            data.applicability_reason = reason
            LOG.warning(
                "  %s: %s -- A=%.4g does not clear the w=1.0 barrier "
    ...
    if data.ccm is not None and data.mu is not None:
        ...
        try:
            ...
            ttc = conversion.ttc_pd(tables, data.ccm, data.mu, pit_pd=data.pit_pd)
            data.ttc_pd = ttc.value
            data.rating_basis = ttc.basis.value
            data.rating_off_grid = ttc.off_grid
            data.ttc_at_floor = ttc.at_floor
            data.sp_risk_score = (
                float(ttc.risk_score) if ttc.risk_score == ttc.risk_score else None
            )
            ...
            if not data.model_applicable:
                ...
                conversion.RatingBasis.GRID_INTERIOR,
                ...
                LOG.info(
                    ...
                    data.outlook,
                    data.rating_basis,
                    "  (TTC at grid floor -- floor-determined)" if ttc.at_floor else "",
                )
            else:
                ...
                data.outlook = None
                LOG.warning(
                    "  rating: %s -- no letter reported (CCM=%.4g, mu=%.4g)",
                    data.rating_basis,
                    data.ccm,
                    data.mu,
                )
    ...
    if (
        cfg.run_bootstrap
        and data.sigma_A is not None
        and data.asset_value is not None
        and _em_asset_path is not None
    ):
        ...
        try:
            ...
            LOG.info(
                "  bootstrap: %d reps, block=%d, defective in %.0f%% of "
                "replicates, rating interval %s..%s (%d notches)",
                b.n_replicates,
                b.block_length,
                100 * b.defective_fraction,
                best,
                worst,
                width,
            )
...
def _log_vol_comparison(companies: list[CompanyData]) -> None:
    ...
    rows = [(c.ticker, c.sector or "?", c.sigma_A) for c in companies if c.sigma_A is not None]
    if not rows:
        return
...
def run(cfg: RunConfig) -> list[CompanyData]:
    ...
    LOG.info(
        "Universe: %s | window: %dy | rates: %s | credit-model: %s",
        ", ".join(cfg.tickers),
        cfg.years,
        cfg.include_rates,
        cfg.run_credit_model,
    ...
    if failures:
        LOG.error(
            "%d ticker(s) raised instead of degrading -- by this "
    ...
    LOG.info(
        "Done. %d/%d companies succeeded. Output: %s",
        len(companies),
        len(resolved),
        dashboard_config.OUTPUT_DIR,
    )
    from creditrating.io import workbook as _wb

    from . import provenance

    manifest_path = provenance.write_manifest(run_id, cfg, companies, _wb.OUTPUT_DIR)
    slog.info(
        "run.done",
