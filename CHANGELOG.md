...
## deliverable-v1 — 2026-07-26

First frozen deliverable. Generated by the single writer
(`dashboard/submission.py`) under the team timestamp standard
(`docs/TIMING_PROTOCOL.md` §10: tz-aware UTC, ISO 8601), archived in
`docs/deliverables/` beside the pre-look-ahead version with a `DIFF.md`.

### Added

- **Ratings sheet** in the workbook: the presentation rule applied — sorted by
  RiskScore, letter only ever with its bootstrap interval attached (or the
  reason there is none).
- **Validation sheet** extended: drift t, explicit `At scale top` flag, rating
  interval, convention span (from the 2026-07-26 debt-weight sweep), and debt
  field provenance (which line item supplied each leg, with a CONTRADICTORY
  flag when the source disagrees with itself).
- **README sheet** extended: applicability gates, convention sweep ranges (and
  which conventions have *not* been swept), the Ratings-sheet reading rule.
- **Never-overwrite guard**: writing onto an existing submission path raises.

### Changed

- **Applicability gate revised (ADR 0003, revision 1):** the negative-book-
  equity gate is replaced by a market-based test — market-implied `A` must
  strictly exceed `ST + 1.0·LT`, the most conservative debt convention, so
  applicability cannot depend on the arbitrary long-term weight. DELL is rated
  again (A−, drift t = 2.01, 10-notch interval); standing is 7/10 rated,
  3/10 scale-resolved, PNC the sole `MODEL_NOT_APPLICABLE`.
- Uncertainty documentation presents the three sources side by side —
  parameter (bootstrap), convention (debt-weight sweep), specification (PNC's
  barrier definition) — with the combined conclusion in `docs/UNCERTAINTY.md`
  and the README results section.

## Pre-freeze (2026-07-25/26)
