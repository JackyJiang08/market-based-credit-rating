...
def _simulate_equity(sigma_true=0.25, eta=0.08, D=100.0, r=0.04, A0=200.0, n=252, seed=7):
...
def test_inversion_raises_when_the_root_cannot_be_bracketed(monkeypatch):
    ...
    with pytest.raises(em_mod.EMError, match="failed to bracket"):
        em_mod._invert_assets(
            np.array([100.0, 200.0]), np.array([50.0, 50.0]), np.array([0.04, 0.04]), 0.3, 1.0
        )
