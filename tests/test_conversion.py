...
needs_tables = pytest.mark.skipif(
    not HAS_TABLES, reason="conversion workbook (proprietary) not present"
)
...
def test_alpha_sp_uses_one_over_q_not_q():
    ...
    L = math.log(ccm + 1.0)
    expected = norm.cdf(
        (1.35 - (1.0 / conversion.Q_SP) * math.log(ccm) + L / 2.0) / math.sqrt(L)
    )
    ...
    wrong = norm.cdf((1.35 - conversion.Q_SP * math.log(ccm) + L / 2.0) / math.sqrt(L))
...
@pytest.mark.parametrize("grade, ccm_md, alpha_pub, ccm_star_pub, rs_pub, pd_ln", TABLE_12)
def test_table_12_alpha_and_ccm_star(grade, ccm_md, alpha_pub, ccm_star_pub, rs_pub, pd_ln):
...
def test_analytical_route_refuses_undefined_inputs():
    for pit, ccm in [(float("nan"), 1.5), (0.1, float("nan")), (0.1, 0.0), (0.1, -1.0)]:
